...
- **2026-09-06** — **Warranty page split into "Check Warranty" + a new "Claim Warranty" workflow.** Client asked to rename "Warranty Check" to "Warranty" with two tabs. Tab 1 is the existing IMEI lookup, unchanged. Tab 2 is a new feature: search by IMEI (business-wide, reuses the same PhoneUnit lookup pattern) auto-fills the product and the customer's full contact details (name/phone/NID/address, via the unit's populated customer ref); if nothing matches, the form stays open for manual entry (a device bought elsewhere, or an old sale not in the system). Submitting creates a new `WarrantyClaim` (claimNo, optional unit/product/customer refs + free-text fallbacks, problem note, status + statusHistory — same shape as `ServiceJob`) and immediately prints a thermal acknowledgement receipt (`WarrantyClaimReceipt.jsx`) the customer keeps as proof. A claims list tracks each one through 4 statuses — At Shop → Sent to Company → Received from Company → Delivered to Customer (client described 3; added "Received from Company" as the natural intermediate step, matching `ServiceJob`'s own 4-status granularity) — changed inline via a per-row select, same pattern as Service/Repair. New `WarrantyClaim` model/controller/routes (mounted at `/api/warranty-claims`), reusing the existing `warranty` staff-permission key rather than adding a new one. Verified: server `node --check` + full import-chain, client `vite build`, and a full route-name trace between the client calls and `warrantyClaimRoutes.js`; not exercised against a live database (standing limitation) — recommended the client submit one real test claim before relying on it.
- **2026-09-06** — **Claim Warranty follow-up — a status dashboard.** Client asked to see at a glance how many phones are in each stage of the claim process. New `GET /warranty-claims/summary` (aggregate count-by-status, scoped like the claims list but deliberately independent of whatever search/status filter is currently applied there). Claim Warranty now opens with 4 stat cards (At Shop / Sent to Company / Received from Company / Delivered to Customer), refreshed after every create/status-change/delete; clicking a card toggles the list below to just that status. Verified: server import-chain, client build, identifier trace.
- **2026-09-06** — **Fix — Claim Warranty dashboard always showed 0 + list was missing NID/Address.** The status dashboard's `$match` used `branchFilter(req)`'s plain string business/branch ids directly inside an `aggregate()` pipeline — unlike `.find()`, `$match` gets none of Mongoose's automatic string→ObjectId casting, so it silently matched nothing. Fixed by wrapping both ids in `new mongoose.Types.ObjectId(...)`, the same workaround `balanceService.js` already applies for its own aggregations. Also added the customer's NID and address as extra lines in the claims list (matching how `Customers.jsx` already renders them), since only name/phone were shown before.
- **2026-09-06** — **Warranty split into a two-item Sidebar sub-menu, matching Branches.** Client asked for Check Warranty / Claim Warranty to sit under "Warranty" the same way "All Branches" / "Stock Transfer" sit under "Branches" — two pages, not in-page tabs. Split the old single tabbed `Warranty.jsx` into `Warranty.jsx` (Check Warranty, unchanged, `/warranty`) and a new `ClaimWarranty.jsx` (`/claim-warranty`); `Sidebar.jsx` gained a collapsible "Warranty" section (same open/close + chevron pattern as Branches), rendered right after Suppliers via a permission-aware link-list split with a safe fallback for a staff login granted 'warranty' but not 'suppliers'. Pure navigation restructure — no change to the Claim Warranty feature itself. Verified: client build, full identifier trace.
- **2026-09-06** — **Claim Warranty: a delivery-confirmation receipt.** Client asked for a second receipt to print automatically once a claim is marked "Delivered to Customer" — proof the device was actually handed back — with every receipt staying on file for a later recheck. New `WarrantyDeliveryReceipt.jsx` (thermal: claim no, submitted/delivered dates from the claim's own `statusHistory`, customer/product details, a plain confirmation line) auto-opens the moment `changeStatus()` sets a claim to `delivered_to_customer`, and a second print icon appears in the claims list (only once a claim has actually reached that status) so either receipt can be reprinted anytime — the claim document with its full history already stays in the database and listed forever, so nothing new needed to persist for the recheck ask. Verified: client build, identifier trace.
- **2026-09-06** — **Products: new "Stock Print by Brands" report + a day-over-day total comparison.** Client wanted a second stock report grouped purely by brand (no supplier) — e.g. all Oppo phones under one running SL# sequence, then Samsung's own sequence — plus a "Today's Total Products vs Last Day's" comparison at the bottom (their example: 180 vs 210). New `StockReportByBrand.jsx` (sibling to `StockReport.jsx`, same A4 layout, grouped by `Product.brand` with a per-group running SL# column, blank-brand items fall into "— No Brand —"). Since nothing tracks a daily stock history, added `StockSnapshot` (business+branch+category+date, unique per day) + `GET /products/stock-snapshot` — upserts today's total-products/total-qty (for the active branch + whatever category is selected) every time the report is generated, and returns the most recent prior day's reading alongside it; the very first run correctly shows "No earlier reading yet" rather than a fabricated number. New "Stock Print by Brands" button on Products (the previously-empty slot next to "Stock Print"). Verified: server import-chain, client build, identifier trace.
- **2026-09-06** — **Fix — Stock Print by Brands wasn't actually grouping.** Client's screenshot showed one continuous SL sequence running straight across obviously different brands. Root cause: this shop's catalog came in almost entirely through Smart Import, which never set `Product.brand` — grouping strictly on that field put nearly everything into one giant "No Brand" bucket. Fixed by falling back to the first word of the product's own name (already formatted "`<Brand> <Model>...`") whenever `brand` is blank, and switched to alphabetical group/item ordering ("ABC serial") instead of by quantity. Flagged one unavoidable limitation: a genuine spelling typo in the source data (e.g. "Smasung" vs "Samsung") still produces two separate groups — auto-merging spellings risks wrongly merging different real brands — but each now shows as its own visible, correctly-labeled group instead of disappearing into one bucket, so a typo is easy to spot and fix via Edit Product.
- **2026-09-06** — **Named Payment Accounts — the shop's real bank/bKash/Nagad sub-accounts.** Client has 5-10 real bank accounts and wants to know exactly which one a payment/transfer moved through, not just "bank" as a whole method (same for multiple bKash/Nagad numbers). New Settings → Payment Accounts (owner/superadmin manage; any staff can read the plain list to pick one) — add/edit/deactivate a named sub-account per method (bank/bKash/Nagad/Rocket/card). An optional `account` reference was wired into every money-movement path the client described: POS checkout (`Sale.payments[]`) + Money Back (`Sale.moneyBacks[]`), Fund (Add/Withdraw), Balance Transfer (upgraded to real account-to-account — bank→bank between two *different* named accounts is now a valid transfer), Expense, and Collect Due (both per-invoice and per-customer). Every place is optional/additive — left blank, everything behaves exactly as before under an "unassigned" bucket per method — so no existing record needed touching. Invoices/receipts and Finance's history tables now show the account name next to the method wherever one was used. New `GET /payment-accounts/balances` (gated to 'finance', more sensitive than the plain list) computes each account's own running balance via a new parallel `balanceService.computeAccountBalances()` — the existing, well-tested per-method `computeBalances()` is completely untouched. Finance page gained a "Bank / Mobile Banking Accounts" section with a dashed "General / Unassigned" card per method for money not tied to a specific account. **Real bug caught and fixed while touching this code**: `customerController.collectDue` (Customers page's own Collect Due, distinct from the per-invoice one) has been creating a `DuePayment` with no `branch` ever since Phase 25 made that field required — `customerRoutes.js` never chained `resolveBranch` — so every customer-level due collection has silently been throwing after the due was already reduced (same bug class as the earlier Pay Salary fix this session). **Deliberate scope decision**: Supplier purchases, EMI/Installment payments and Service job payments do NOT carry an `account` field in this pass (the client's own examples were POS/Fund/Transfer/Expense/Collect-Due) — add it the same way (a field + `resolveAccountId()`) if the client wants full coverage later. Verified: `node --check` across the entire server tree + full `app.js` import-chain; client `vite build`; an 8-case logic fixture mirroring the account-balance net-sign math (sale payments, fund add/withdraw, an account-to-account transfer, expense+due-collection net, money-back, a full lifecycle, and the unassigned-bucket derivation) — 8/8; a full identifier trace of every new `AccountSelect`/account-state reference across POS/Finance/Customers/OrderDetailsModal/Settings. Not exercised against a live database — recommended the client add their real accounts in Settings and run one test sale/expense/transfer with an account picked to confirm it shows correctly everywhere.
- **2026-09-06** — **Fix — Groq AI calls failing: "model `llama-3.3-70b-versatile` does not exist".** Client's Dashboard "Generate summary" errored with exactly that. Checked directly against Groq's own `/v1/models` with the client's real key — every plain llama-3.x chat model has been removed from Groq's lineup entirely (not a key/access issue); current lineup is `openai/gpt-oss-*`, qwen, allam, and the agentic `groq/compound` models. Switched the default to `openai/gpt-oss-120b` (both hardcoded spots in `aiService.js`, still overridable via `GROQ_MODEL`); added `reasoning_effort: 'low'` plus a fallback to the model's `reasoning` field when `content` comes back empty, since the gpt-oss family can put its answer there under a tight token budget. Verified against the real Groq API with the client's actual key end-to-end, including running the exact prompt shape + 600-token budget `dashboardController.aiSummary` sends — clean two-part response, not cut off.
- **2026-09-06** — **UI redesign — teal + coral color scheme.** Client felt the app was too plain/white; showed 8 mockup palette options (two rounds of 4) via the visualize tool, client picked "teal sidebar with coral accents". `tailwind.config.js`'s `brand` ramp (used everywhere via `bg-brand-*`/`text-brand-*` — buttons, active states, focus rings, spinners, badges) switched from indigo to a full teal ramp (50–950, filling gaps the old 5-stop ramp never defined — caught a latent bug where Sidebar's dark-mode active-label class referenced a `brand-400` Tailwind was silently never generating); new `coral` ramp added as a deliberate second color used sparingly (Sidebar's active nav item, one Dashboard balance card). `Sidebar.jsx` restyled to a fixed dark-teal panel independent of the app's own light/dark content toggle (which still governs the main content area) — matching the picked mockup exactly. `RevenueChart` and `index.html`'s theme-color meta updated from the old indigo hex to teal. Verified: client `vite build`; grepped all 56 `brand-*`/`coral-*` references in touched files against the new config's defined stops. Not visually verified live (needs the client's real login/data) — asked the client to hard-refresh and confirm.
- **2026-09-06** — **Products: new "Stock Print by Model" report.** Third stock report, drilling into a single product instead of listing many: click the button, search by name (suggestions show category/supplier/stock), pick one, get a printable report for just that model. New `GET /products/:id/report` returns total pieces sold all-time (net of returns, same rule Phase-28's `supplierProductBreakdown` already applies), every supplier this exact product was ever bought from with quantity + last purchase date, and current stock. New `ProductStockReport.jsx` (A4, sibling to the by-supplier and by-brand reports, but a single-product drill-down instead of a list) — identity block, three summary boxes, a supplier-wise table, buy/sell price. Verified: server import-chain, client build, a 4-case logic fixture on the sold/supplier-grouping arithmetic, identifier trace.
- **2026-09-06** — **Stock Print by Model — added a date-wise sale history.** Client wanted exactly which date each unit sold, not just the running total. `GET /products/:id/report` now also returns `sales`: one row per matching `Sale.items` entry (a serial-tracked product sold as several devices in one invoice gets a row per device) with date, invoice, customer, IMEI/serial, qty (noting any returned amount), and sale price — sorted newest first, uncapped since it's a print report. `ProductStockReport.jsx` gained a "Sold On" table between the supplier breakdown and the price line. Verified: server import-chain, client build, prop trace.
- **2026-09-06** — **Returns & Exchange: search-driven "New Return/Exchange" tab + a printed receipt.** Client wanted every sold invoice reachable straight from this page — search by invoice number or customer phone, see the date, act — plus a real customer-facing receipt for what an exchange settled to. `Returns.jsx` restructured into two tabs: "New Return / Exchange" (a search box against the existing `GET /sales/search` — same lookup Invoice Search uses — opening a result's full invoice, which already carries the Return/Exchange button) and "History" (the previous page, unchanged — already covered "which products have been returned/exchanged so far"). `GET /sales/search` now also populates the customer's phone (shown in both this new tab and Invoice Search). New `ReturnReceipt.jsx` (thermal) prints automatically the moment a return/exchange completes — items handed back with condition (resellable → back to stock, stated plainly), how the value was settled (due/refund/store credit), and for an exchange specifically the new invoice's replacement item plus whether the customer paid extra or got money back — closing Phase 9's own explicitly-flagged gap ("no dedicated return/exchange print receipt... can add if the client wants one"). Verified: server import-chain, client build, identifier trace.
- **2026-09-06** — **Returns & Exchange: Sidebar sub-menu (like Warranty) + a full browsable order list.** Client's screenshot pointed at Warranty's own two-item Sidebar sub-menu and asked Returns & Exchange work the same way instead of in-page tabs, plus wanted "New Return / Exchange" to show every sold invoice (not just Dashboard's 6 recent), searchable by invoice/phone/date. `Sidebar.jsx` now splices in a collapsible "Returns & Exchange" group (New Return / Exchange → `/returns`, History → `/return-history`) at the same spot the flat link used to sit — extracted the repeated group button+submenu JSX (now duplicated three times with Warranty and Branches) into a small `NavGroup` component. Split the page: `Returns.jsx` (route unchanged) is now the full browser — paginates every branch invoice by default (20/page, newest first), switches to the existing business-wide `/sales/search` once 2+ characters are typed, date range narrows the paginated view; new `ReturnHistory.jsx` takes the old History tab content at `/return-history`. `GET /sales` gained real pagination (page/pageSize/total) and now populates the customer's phone (client asked for the number to show). Verified: server import-chain, client build, identifier trace.
- **2026-09-06** — **Employees: a "View Buy Price" toggle.** Client wants some staff to have Products access without seeing purchase cost. New permission key `view-buy-price` added to both module lists — not a page, so it shows in the existing "Login Access" checkbox grid for free, granting nothing on its own; owner/superadmin unaffected. Enforced server-side (not just UI-hidden): a restricted staff login gets `purchasePrice` redacted to `null` (distinct from a genuinely-unset ৳0) in `GET /products`, the update response, and `GET /products/:id/report`; `PUT /products/:id` also silently drops any `purchasePrice` a restricted request tries to set, since their blank Edit-form field must never overwrite the real stored price. `Products.jsx` hides the Buy column, the price-search card's Buy box, the per-unit profit line (which would let them back-calculate buy price from sell price otherwise), and the Edit form's Purchase Price field — the create-mode Add Product form is untouched, since entering a new product's own price isn't "viewing" someone else's data. Verified: server import-chain, client build, a 9-case logic fixture on the permission check and the null-vs-zero distinction.
- **2026-09-06** — **UI polish — brought the teal theme into the content area, not just the sidebar.** Client's screenshots (Dashboard, Products, POS) showed every page's own canvas — body background, cards, table headers, Topbar — still flat slate/white, disconnected from the new sidebar theme. Changed a small set of shared classes so every screen updates at once: `body` background `bg-slate-50` → `bg-brand-50` (light mode only — a faint teal wash under white cards, standard "colored canvas + white content" pattern); `.card`'s border `slate-200` → `brand-100` (every card/table/panel in the app uses this one class); `DataTable`'s header row `bg-slate-50` → `bg-brand-50`; `Topbar`'s bottom border `slate-200` → `brand-100`. Deliberately left `.input`, `.btn-ghost`, and modal panels neutral — tinting every button/field risked looking busy rather than "a bit of color". Verified: client build, grepped the new classes against `tailwind.config.js`'s defined stops.
- **2026-09-06** — **UI polish — deepened list/table border lines.** Client's screenshots showed row-separator lines nearly invisible against the now-teal-tinted page (the old `border-slate-100` hairline barely showed on white rows). `DataTable.jsx` (every list page's shared table) — row border `slate-100` → `brand-200`, row hover `slate-50` → `brand-50`; `.card`'s border `brand-100` → `brand-200`; `Topbar`'s bottom border matched. Every other hand-rolled table/list-divider not going through `DataTable` (Suppliers' ledger/breakdown tables, Installments' schedule, ReturnExchangeModal/OrderDetailsModal's item tables, NotificationBell's dropdown, Dashboard's activity list, Products' unit-management tables and its Stock-Print-by-Model suggestion dropdown) got the same mechanical `border-slate-100`/`divide-slate-100` → `brand-200` swap, so every list in the app reads consistently. Verified: client build, grepped every new class reference across all touched files.
...
- **2026-09-07** — **Full-system audit, then fixed all three problems it found.** Client asked for every function to be checked end to end ("full system ta ak ak kore test kore amake akta result dao"). Audited: the `branch` field on all 25 model-creation sites, the middleware chain on all 30 route files, tenant isolation, all 149 client→server API calls, and a 14-scenario/56-check logic fixture mirroring each controller's exact arithmetic (sale totals/discounts/split payments/stock, due collection + customer ledger, resellable vs damaged returns, over-return guard, exchange in both directions, genuine money-back, salary split-tender, service jobs, EMI payment-by-payment profit, fund/transfer/expense/purchase, and a whole-day money-conservation run). Everything passed except three real problems, all three now fixed:
  1. **Phantom "Money Back" after any return/exchange — could pay a customer twice.** A return lowers `total` but deliberately never lowers `paid` (that's the real till cash, and the balance engine reads it directly for legacy single-tender sales, while the refund leaves separately via `Return.cashRefund`). But `moneyBackOf()` is derived as `paid − total − moneyBackReturned`, so the moment `total` dropped, the difference read as an overpayment still owed — and `OrderDetailsModal` offered a one-click payout for the full returned value. Fixed with a new `Sale.returnCredit` counter: `processReturnItems` banks `returnValue − dueReduction` (the part that came out of already-paid money) and `moneyBackOf()` subtracts it. `paid`/`payments[]` stay untouched, so no balance-engine path changed and no existing record needed migrating. A genuine overpayment on an invoice that also had a return is still reported correctly.
  2. **Profit came back when a returned invoice was edited.** `updateSale` recomputed `profit` from raw line items ignoring `returnedQty`, so opening a returned invoice and saving it — even unchanged — restored the full pre-return margin. Quantities are now counted net of returns (`total` was already right, since the return itself reduces `subTotal`).
  3. **"View Buy Price" was bypassable via four other routes.** The permission only covered the Products list, its update response and the single-product report. Also closed: `getProductByBarcode` (the most important — the route counter staff hit on every scan, under the plain `products` permission), the products/sales/purchases exports (the JSON format returns raw rows and ignores the column list, so dropping a CSV column was never enough; `Sale.profit` and `Purchase.items[].unitCost` give cost away just as directly), the Advanced Report (`stockValue` ÷ stock = the exact buy price; same for a per-product profit beside its revenue and qty — both blanked, shop-level totals left alone since `finance` already grants those), and the Dashboard's low-stock widget (shipped whole Product documents). The **full backup is refused outright** rather than redacted — it dumps every collection and a cost-stripped one would silently be useless to restore from. The gate moved out of `productController` into **`server/src/utils/buyPrice.js`** so all five controllers ask the same question; `AdvancedReport.jsx` renders a dash for a hidden figure so it never reads as a real ৳0.
